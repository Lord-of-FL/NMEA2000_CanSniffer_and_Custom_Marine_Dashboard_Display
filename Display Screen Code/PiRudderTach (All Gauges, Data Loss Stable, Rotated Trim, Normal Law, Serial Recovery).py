...
# If the serial reader goes stale while the Arduino is still alive,
# force a clean serial reconnect periodically.
SERIAL_RECOVERY_SECONDS = 3.0

ser = None
last_serial_try_time = 0.0
last_good_frame_time = 0.0
last_serial_recovery_time = 0.0
...
def process_serial_data():
    ...
    try:
        ...
        # Drain all waiting serial lines and keep only the newest one.
        # This prevents the Pi from slowly falling behind or getting stuck
        # processing old/partial junk if the serial buffer gets weird.
...
def draw_control_regime_box(surface):
    # Top-left placement
    margin = int(smaller_dim * 0.02)

    # Fonts (match mock: small white label + green value)
    label_font = pygame.font.SysFont("Arial", int(smaller_dim * 0.028), bold=False)
    value_font = pygame.font.SysFont("Arial", int(smaller_dim * 0.032), bold=True)

    label_surf = label_font.render("Control Regime", True, WHITE)
    value_surf = value_font.render("NORMAL LAW", True, GREEN)

    # Box sizing / padding
    pad_x = int(smaller_dim * 0.012)
    pad_y = int(smaller_dim * 0.010)
    spacing = int(smaller_dim * 0.006)
    corner = int(smaller_dim * 0.012)

    box_w = max(label_surf.get_width(), value_surf.get_width()) + 2 * pad_x
    box_h = label_surf.get_height() + spacing + value_surf.get_height() + 2 * pad_y

    x = margin
    y = margin

    # Draw box directly on the main surface so it's guaranteed visible
    pygame.draw.rect(surface, BLACK, (x, y, box_w, box_h), border_radius=corner)

    # Text
    surface.blit(label_surf, (x + pad_x, y + pad_y))
    surface.blit(value_surf, (x + pad_x, y + pad_y + label_surf.get_height() + spacing))
...
while running:
    ...
    # AND periodically kick/reopen the serial port in case the Pi reader got stuck.
    now = time.monotonic()
    if now - last_good_frame_time > DATA_STALE_SECONDS:
        set_no_data_state()

        if now - last_serial_recovery_time > SERIAL_RECOVERY_SECONDS:
            print("Serial stale - forcing serial reconnect")
            close_serial()
            last_serial_recovery_time = now
    ...
    # --- Aviation Easter Egg (draw LAST so nothing covers it) ---
    draw_control_regime_box(screen)
